...
FileBlock* fillFileBlocks(MetaFileInfo* metaFileInfo)
{
    ...
    if (hash_found == 0)
    {
        ...
        {
            readBlock(&fileBlocks[i],metaFile);
            if (i < 1000)
            {
                printBlock(&fileBlocks[i]);
            }

            if (i > metaFileInfo->originalFileBlocksStart - 1000)
            {
                printBlock(&fileBlocks[i]);
            }
        }
        metaFileInfo->fileBlocksCount = metaFileInfo->filesCount;
        metaFileInfo->fileBlocksStart = metaFileInfo->originalFileBlocksStart;
...
/// ******************* FOLDER NAMES DECRYPTION **********************
    ...
    const uint8_t *decrypction_key = "\x51\xF3\x0F\x11\x04\x24\x6A\x00"; // The Black Desert ICE decryption key
...
void readBlock(FileBlock* fileBlock, FILE* fp)
{
    ...
    long zsize;
    long size;
    fread(&fileOffset,sizeof(long),1,fp);
    fread(&zsize,sizeof(long),1,fp);
